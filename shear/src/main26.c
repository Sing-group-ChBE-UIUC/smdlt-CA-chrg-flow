...
int main (int argc,char * argv[]){
	...
	for(itercount=iterstart;itercount<itermax;itercount++){
		...
		sprintf(rgt, "prop/RG%d_%d_%.6lf_%.6lf_%d_%.2lf_%.2lf_%.2lf_%.5lf_%d_%d.txt", Nb, Nc, c_norm, L,Ncharges,barrier,barrier2,lambda_d,flowrate, trace,itercount);
	    ...
	    for(t = tstart; t<(tmax+1); ++t){
	    	...
	    	if(flowtype!=0){
				...
				if(t%printprops==0){
                    ...
                    calcRg(); // calculate gyration tensor
					printExt();
					//printTiming();
				}
				checkVerlet();
				...
				if(itercount==0){
                    ...
                    //printf("before updateHI\n");
					updateHI();
                    //printf("after updateHI\n");
				}
                if(chargehop_type==1){
					chargeHop();
				}
...
void updateFD(){
	...
	for(i=0;i<N;i++){
		dx = 0.0; dy = 0.0; dz = 0.0;
		dx = dt*(fx[i] + flowrate*ry[i]) + pc*R[3*i];
...
void calcRg(){
	...
	for(i=0;i<Nc;i++){
		for(j=0;j<Nb;j++){
			rgp[0] += (px[Nb*i+j]-comx[i])*(px[Nb*i+j]-comx[i]); // rgxx
			rgp[1] += (px[Nb*i+j]-comx[i])*(py[Nb*i+j]-comy[i]); // rgxy
			rgp[2] += (px[Nb*i+j]-comx[i])*(pz[Nb*i+j]-comz[i]); // rgxz
			rgp[3] += (py[Nb*i+j]-comy[i])*(py[Nb*i+j]-comy[i]); // rgyy
			rgp[4] += (py[Nb*i+j]-comy[i])*(pz[Nb*i+j]-comz[i]); // rgyz
			rgp[5] += (pz[Nb*i+j]-comz[i])*(pz[Nb*i+j]-comz[i]); // rgzz
		}
		rgp[0] /= (double)Nb; rgp[1] /= (double)Nb; rgp[2] /= (double)Nb;
		rgp[3] /= (double)Nb; rgp[4] /= (double)Nb; rgp[5] /= (double)Nb;

		fprintf(rgtfile,"%lu %d %lf %lf %lf %lf %lf %lf\n",t,i,rgp[0],rgp[1],rgp[2],rgp[3],rgp[4],rgp[5]);
	}
	fclose(rgtfile);
}
void printExt(){
...
void ewaldBin(){
    ...
    for(kx=0;kx<kxmax+1;kx++){
        ...
        for(ky=-kymax;ky<kymax+1;ky++){
            rky = kcoeff*(kx-ky*(L2[0]/L1[0]))/L2[1];
			rkx = kcoeff*ky/L1[0] ;
...
void updateLattice(unsigned long t){
	...
	L1mag = pow(L1[0]*L1[0]+L1[1]*L1[1],0.5);
	L2mag = pow(L2[0]*L2[0]+L2[1]*L2[1],0.5);
	theta = acos((L1[0]*L2[0]+L1[1]*L2[1])/(L1mag*L2mag));
	// printf("theta: %lf\n",theta*180/3.1415);
	L1xp = L1[0];
	L2xp = L2[0];
	L1yp = L1[1];
	...
	//printf("t: %lu L1x: %lf L1y: %lf L2x: %lf L2y: %lf theta: %lf\n",t,L1xp,L1yp,L2xp,L2yp,theta*180/3.1415);
	// exit(1);
}
void applyPBC(){
	int i;
    int treal = t%tp;
	double delx = flowrate*treal*dt ; //slightly different than what TD says (p. 120)
    //printf("delx: %lf\n",delx);
    for(i = 0; i<N; ++i){
...
Vector3D_t getNIDbin(int i,int j){
	Vector3D_t NID;
	int nx,ny,nmax;
	double dx,dy,dz,dxp,dyp,theta,dxL2,dxmin,dymin,rp,rmin,delx;
	theta = atan(L1[1]/L1[0]);
				...
				//Actual Monte Carlo Test
					if (1<=exp((-barrier-0.5*(E_final - E_initial))))
					{
						count=0;
										...
										dxt[k*printprops+tcount] += dx;
										dyt[k*printprops+tcount] += dy;
...
void calcDisFD()
{
	...
	for(i=0;i<Ncharges;i++){
        ...
        dx = dt*(fx[j] + flowrate*drelyi) + pc*R[3*j] ;
		dy = dt*fy[j] + pc*R[3*j+1];
