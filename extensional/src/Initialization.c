...
void initBox(){
    ...
    conc = c_norm*c_star;
    // if(Nb==100 && Nc==13){
    //     printf("Warning - box length is hard-coded\n");
    //     if(c_norm==1){
    //         L = 80;//78.934411 ;
    //     }
    //     else if(c_norm==5){
    ...
    //     }
    //     else{
    //         printf("concentration not recognized, exiting");
    //         exit(1);
    //     }
...
void initEwald(){
    ...
    if(iterstart>0){
        ...
        sprintf(decomp, "decomp/DC%d_%d_%.6lf_%.6lf_%d_%.2lf_%.2lf_%.2lf_%.5lf_%d_%d.txt", Nb, Nc, c_norm, L,Ncharges,barrier,barrier2,lambda_d,flowrate, trace,iterstart-1);
